...
const sign_up = ({username, phone, response}) =>
{
    if (username.length >= 4 && phone.length === 11)
    {
        ...
        request.query(`select phone from users where phone = N'${phone}' or username = N'${username}'`, (error, records) =>
        {
            ...
            else
            {
                ...
                {
                    response.send({state: -4, log: "PHONE_NUMBER_OR_USERNAME_IS_ALREADY_EXIST"})
                }
                else
                {
...
const update = ({id, name, phone, email, response}) =>
{
    if (!isNaN(id))
    {
        let request = new mssql.Request(Connection.connection)
            ...
            , (error, records) =>
            {
                ...
                else
                {
                    if (records && records.recordset.length > 0)
                    {
                        ...
                        {
                            request.query(
                                `update users set name = N'${name}', email = N'${email}' output inserted.* where id = '${id}'`
                                , (err, rec) =>
                                {
                                    if (err) response.send({state: -6, log: "DATA_BASE_ERROR", form: err})
                                    else response.send({
                                        state: 1,
                                        log: "SUCCESSFUL_USER_UPDATE",
...
const select_admin = ({id, response}) =>
{
    ...
    else
    {
        let request = new mssql.Request(Connection.connection)
        request.query(
            `select * from users where id = N'${id}'`
            ...
            else
            {
                if (records.recordset.length > 0)
                {
                    records.recordset[0].type === "Admin" ?
                        request.query(
                            `select type from users where id = N'${user_id}'`
                                ...
                                else
                                {
                                    if (recordss.recordset.length > 0)
                                    {
                                        recordss.recordset[0].type === "Person" ?
                                            request.query(
                                                `delete from users where id = N'${user_id}'`

                                                , (error0) =>
                                                {
                                                    if (error0) response.send({
                                                        state: -10,
                                                        log: "DATA_BASE_ERROR",
                                                        form: error0,
                                                    })
