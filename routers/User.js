...
const {decode} = require("../functions/token")
...
user_router.route("/admin/")
    .get((req, res) =>
    {
        ...
        else if (header.authentication !== undefined)
        {
            const callback = (payload) => // payload: Decoded Data(id, username, phone, iat)
            {
                select_admin({id: null, response: res})
            }
            decode({
    ...
    .get((req, res) =>
    {
        ...
        if (header.auth !== undefined)
        {
            if (header.auth === Data.auth)
            {
                ...
                else res.send({state: -3, log: `GET_USER_${req.params.id}_IS_NOT_NUMBER`})
            }
            else res.send({state: -2, log: "GET_USER_INCORRECT_HEADER"})
        }
        else if (header.authentication !== undefined)
        ...
        else res.send({state: -1, log: "GET_USER_PERMISSION_DENIED", form: header})
    })
